...
- 新增资料保存后支持四种流转：保存后查看详情、保存并建立向量索引、保存后返回该 Unit 覆盖、保存+索引后返回覆盖。
- 覆盖统计修复：占位资料（标题/标签含“资料占位”且 `doc_type=其他`）不再计入教材/Reading/Writing/Vocabulary/教案覆盖与索引数量。
- 新增 `tests_round_023.py`，覆盖补充入口渲染、预填逻辑、保存后返回覆盖页，以及“占位资料不算已覆盖”回归。

## 0.22.6
- 优化 `/knowledge` 卡片页整体居中与宽度：新增统一页面壳容器（`max-width: 1320px` + `margin: 0 auto` + 内边距），消除列表贴左与右侧大面积空白问题。
- 统一筛选区与资料卡片区宽度，保证顶部筛选卡片和下方资料卡片在同一布局网格中显示。
- 优化资料卡片三栏比例：主信息区约 45%、状态区约 35%、操作区固定 180px，操作按钮不再挤压正文。
- 增强响应式布局：宽屏三栏、中屏“内容+状态/操作”分栏、小屏单列堆叠，避免横向滚动。
- 优化卡片视觉层级：白底浅边框、16px 圆角、轻微 hover 阴影、标题与元信息字号分层。
- 新增 `tests_round_0226.py`，验证知识库页面容器类、卡片渲染、操作按钮、占位资料 badge 与相关页面可达性。

## 0.22.5
- 将 `/knowledge` 列表页从超宽表格改为卡片式资料视图，按“主信息 / 状态 / 操作”分区展示，降低信息密度并提升可读性。
- 新增占位资料识别与展示优化：`doc_type=其他` 且标题/标签含“资料占位”时标记为占位卡片，显示“等待补充资料”，避免将 `word_count=0`、`not_indexed`、`chunk=0` 误判为异常。
- 新增占位资料筛选：支持“隐藏占位资料 / 只看占位资料”。
- 优化整本教材卡片提示：显示“整本教材”与“建议按 Unit 补充单元资料”，并保留可索引可检索状态表达。
- 操作按钮改为卡片内固定可见紧凑按钮组（查看、下载、索引/重索引、重解析、建议元信息、删除），不再依赖最右侧滚动列。
- 增加 `/knowledge` 页面顶部治理说明与入口信息，明确“知识库治理 / 资料覆盖情况”用途。
- 新增 `tests_round_0225.py`，覆盖知识库卡片页可达性、占位资料识别、整本教材 badge 与关键操作按钮渲染。

## 0.22.4
- 优化 `suggest_metadata` 建议策略：当前 `grade`/`textbook`/`volume`/`unit`/`lesson_type` 已有效且无更可靠规则命中时，不再出现“当前值 → 空值”的降级建议。
- 整本教材识别后采用保守课型建议（`Other`/通用），避免将整册资料激进误判为 `Writing`/`Reading`；`Writing` 仅在标题或文件名明确包含 `Reading for Writing` 时建议。
- 修复“索引成功后仍提示需要重索引”：`documents_need_reindex()` 对历史已索引且 `chunk_count>0` 但缺失 `last_indexed_text_hash` 的资料自动补写 hash，hash 一致时不再标记重索引。
- 知识库详情页“建议元信息”改为“当前值 / 建议值 / 是否建议修改”视图，同值显示“无需修改”，并补充整本教材的课型与拆分提示。
- 更新 `tests_round_022.py`，补充元信息建议保守性、整本教材课型建议、`Reading for Writing` 触发条件、hash 回写和详情页重索引提示回归测试。

## 0.22.2
- 修复知识库列表页右侧操作列不可见问题：增加更宽页面容器与表格横向滚动，确保可滚动查看最右侧操作按钮。
- 为操作列增加右侧 sticky 显示（含表头），并优化为紧凑竖向小按钮组，避免被内容列挤掉。
- 调整知识库列表各列宽度与最小表格宽度，减少非关键列占用，避免右侧列被裁切。
- 增加“可横向滚动查看右侧操作按钮”提示，提升可发现性。

## 0.22.1
- 修复知识库列表页字段扩展后的显示重叠问题，重整 `/knowledge` 表格列宽与信息分组，避免标题、资料信息、治理、索引状态相互挤压。
- 资料信息、解析信息、治理信息、索引状态改为合并展示，减少列数量与密度。
- 增强知识库表格横向滚动与最小宽度（`table-responsive` + `min-width`），在窄屏下避免内容覆盖。
- 优化治理 badge 与操作按钮布局，支持紧凑竖向按钮组与清晰状态标识。
